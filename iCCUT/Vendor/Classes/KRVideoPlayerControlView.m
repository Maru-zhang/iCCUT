...
- (void)layoutSubviews
{
    ...
    self.downloadButton.frame = CGRectMake(CGRectGetWidth(self.bottomBar.bounds) - CGRectGetWidth(self.fullScreenButton.bounds), CGRectGetHeight(self.bottomBar.bounds)/2 - CGRectGetHeight(self.fullScreenButton.bounds)/2, kVideoControlBarHeight, kVideoControlBarHeight);
}
...
- (UIButton *)downloadButton {
    if (!_downloadButton) {
        ...
        [_downloadButton setImage:[UIImage imageNamed:@"media_download"] forState:UIControlStateNormal];
        CGFloat inset = kVideoControlBarHeight * 0.2;
        _downloadButton.contentEdgeInsets = UIEdgeInsetsMake(inset, inset, inset, inset);
    }
